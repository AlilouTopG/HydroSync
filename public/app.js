...
(function () {
  ...
  // GIS Fleet Map Engine Variables
  ...
  function bindControls() {
    ...
    if (shutoff) shutoff.addEventListener("click", function (e) {
      ...
      if (socket && socket.connected) socket.emit("client:emergency_stop");
    ...
    if (rs) rs.addEventListener("click", function (e) {
      ...
      if (!isOperatorAuthorized) {
        openModal("authModal");
        log("<strong style='color:var(--warn)'>[ACCESS DENIED]</strong> Operator authentication required to reset safety trip.");
        return;
      }
      if (socket && socket.connected) {
