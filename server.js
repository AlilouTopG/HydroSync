...
Operator Query: "${message}"`;
  ...
  try {
    ...
    // Full detail server-side; a safe, informative (but not internals-leaking) summary to the client.
    ...
    res.status(status).json({
      reply: 'Cloud AI unavailable. Operating in fallback monitoring mode.',
      error: status === 503 ? 'AI_NOT_CONFIGURED' : status === 504 ? 'AI_TIMEOUT' : status === 502 ? 'AI_MODEL_UNAVAILABLE' : 'AI_ERROR'
    });
  }
});
